...
async function processLayoutDocument(canvas) {
  ...
  status.innerText = "Analyzing layout...";
...
function processTexts(text) {
  processedText = text.replace(/\s+/g, " ").trim(); // Normalize whitespace
  ...
  processedText = processedText.replace(
    /((?:[^\s]\s){2,}[^\s])/gu,
    (match) => match.replace(/\s+/g, "") // Remove spaces between single letters/characters in all languages
  );
